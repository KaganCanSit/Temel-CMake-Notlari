...
Bu dokümanda, CMake'in betik dilinin temel prensiplerine ve kurallarına göz atacağız. CMake'in dil yapısına anlamak, otomasyon, bağımlıkların yönetimi ve çapraz platform desteğinin sağlanması gibi konularda bizlere yardımcı olacaktır.
...
Bu kullanımın yanı sıra bir değişken tanımlayıp dosya içerisinde set etmeden gerekli parametreyi terminal ekranından geçebilirsiniz.
...
**Güvenilir kullanım için değişken atamalarında ve yazdırılmasında tırnak ile birlikte yazılması önerilir!**
...
Burada **SET** fonksiyonuyla ile **USER** değişkeni dosya içerisinde de atanabilir!**

## İfadeler Bir Komuttur ve Geri DÖnüş Değerleri Bulunmaz
...
Matematiksel işlemleri gerçekleştirmek üzere **math** komutu bulunur. İlk argüman **EXPR**, ikinci argüman **sonucun atanacagi degisken**, üçüncü argüman ise gerçekleştirilecek ifadedir.
...
Burada da önceki örnekte olduğu gibi dışarıdan parametre alınarak işlem gerçekleştirilebilir.
...
İfadelerin karşılaştırılması için çeşitli şartlar tanımlıdır. Örneğin AND, OR, EQUAL, LESS, LESS_EQUAL, GREATER, GREATER_EQUAL, STREQUAL... Kapsamlı olarak içerik [CMake resmi sitesinde](https:://cmake.org/cmake/help/latest/command/if.html) yer almaktadır. Ayrıca ilerleyen örneklerde de değineceğiz.

    if(UNIX) # WIN32 veya MSVC kullanilabilir
        message("Su an Linux isletim sisteminde CMake'i calistirmaktasiniz.")
    endif()

Benzer bir kondisyon şartı örneği;
...
CMake'de bi fonksiyon ve makro tanımlamak koşul şartı yazmaya benzer bir yapıya sahip.
**funciton/endfunction - macro/endmacro** şeklinde kullanılabilirler.

Fonksiyonlar kendi kapsamlarında çalışmaktadırlar, global olarak parametreleri etkileyemezler. Bir fonksiyondan dönüş değeri alabilmek için dışarıdan değişkenin parametre geçilmesi gerekir. Geçilen parametre **PARENT_SCOPE** brayrağıyla doldurulabilir.
...
Makrolar, işleyiş ve yapı olarak fonksiyonlara benzerler. Fakat makrolar, çağrı yapılan kademeyle aynı kapsamda çalışır. Bu sebeple değişken değerleri direk olarak **PARENT_SCOPE** için set edilir.
...
İki kullanımda aynı çıktıyı üretir. Aralarında bulunan temel fark değişken içeriklerini tanımlandığı kapsam alanıdır.
...
Kütüphane örneğinde kullandığımız gibi CMake dizin yapısında yer alan **CMakeLists.txt** komut dosyası aracılığıyla kapsamları yürütür. **add_subdirectory** komutuyla kullanılan bu yöntem çeşitli alt kütüphaneleri ve içerikleri projenize dahil etmenizi sağlar.

Bunun yanı sıra **find_package** komutu yardımıyla dış kitaplıkları arayabilir ve projenize dahil edebilirsiniz. Açıklama ve örnek için içeriğin başında verilen kaynağı daha detaylı okuyabilirsiniz.
...
## Farklı Bir CMake Dosyası Özelliklerini Almak ve Ayarlamak

CMake dosyanızda **add_executable**, **add_library**, **add_custom_target** gibi komutlar aracılığıyla içerikler dahil edebildiğimizi biliyoruz. Bu içeriklerin özelliklerini **get_property** ve **set_property** komutlarıyla okuyabilir, değiştirebilirsiniz.
